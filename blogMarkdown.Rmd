...
#senti_combined <- readRDS("./data/afinn_combined.rds")#FAKE
...
#all_average <- readRDS("./data/average_sentiment.rds")###FAKE!!!
...
server <- function(input, output, session) {
 ...
 sentiDataBar <- reactive({
      ...
      senti_data <- all_average[all_average$year >= input$senti_year[1]
                                    ...
                                    & all_average$debate_type == input$debate_type
  ...
  output$sentiplot <- renderPlot({
    ...
    p6 <- ggplot(senti_set, aes(x=speech_date, group = debate_type, col = debate_type))
    
    p6 + geom_smooth(aes(y=sentiment, linetype = debate_type, col=debate_type), size=1.5, formula=y ~ log(x)) +
 ...
 output$sentibar<-renderPlot({
    ...
    p8 <- ggplot(senti_bar_data, aes(x=year, y=sentiment, group = debate_type, col = debate_type, fill=debate_type, width=.75))
...
server <- function(input, output, session) {
 ...
 output$senti_gov <- renderPlot({
    ...
    p6 <- ggplot(senti_set, aes(x=speech_date))
...
p8 <- ggplot(senti_bar_data, aes(x=year, y=sentiment, group = interaction(debate_type, government),
...
### By Party
<a name="party_dis"></a>
These charts show the scores for all debate and disability debate, seperated between different parties, from 2000 <!-- Check date here!--> to the end of 2016. Conservatives and Liberal Democrats are their own parties, Labour and Labour Co-op MPs are grouped together as Labour. All other parties are grouped together as Other. <!-- As and when, add SDP to liberal democrats -->
...
server <- function(input, output, session) {
 ...
 output$senti_party <- renderPlot({
    ...
    p6 + geom_smooth(aes(y=sentiment, group = interaction(debate_type, party_group),
                         linetype = debate_type, col = party_group), size=1.5, formula=y ~ log(x)) +
      scale_x_date(date_breaks = "2 year",date_labels = "%Y", name = "Date") + 
      scale_y_continuous(name="Sentiment Score") + 
      scale_linetype_manual(values=senti_line) +
...
p9 <- ggplot(senti_bar_data, aes(x=interaction(year, party_group, lex.order = TRUE), y=sentiment, group = interaction(debate_type, party_group),
...
There are three <!-- There are almost certainly more --> options for sentiment analysis:
...
4. The Hu & Liu dataset.

5. The Sentiwords dataset.
...
This app was constructed using [`shiny`](http://shiny.rstudio.com/). Plots are produced with `ggplot2` and `scales`. Sentiment analysis was conducted using the [`sentimentr`](https://cran.r-project.org/package=sentimentr), [`syuzhet`](https://cran.r-project.org/package=syuzhet), [`lexicon`](https://cran.r-project.org/package=lexicon) and [`RSentiment`](https://cran.r-project.org/package=RSentiment) packages.
